...
const buttonThemes: ButtonTheme = {
  base: 'active:focus:scale-95 duration-150 transition-all rounded-md flex items-center justify-center ring-offset-0 focus:outline-none focus:ring-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2',
  color: {
    primary:
...
const Button: FC<ButtonProps> = ({
  ...
  const classNames = {
    base: buttonThemes.base,
    size: !shape && buttonThemes.size[size],
    iconBtn: (shape === 'icon' || shape === 'circle') && buttonThemes.iconBtn[size],
    roundedFull: shape === 'circle' && 'rounded-full',
    color: !variant && buttonThemes.color[color],
    link: variant === 'link' && buttonThemes.variant.link[color],
    outline: variant === 'outline' && buttonThemes.variant.outline[color],
    position: position && buttonThemes.position[position],
  }
  ...
  return (
    <button type={type} disabled={disabled} onClick={onClick} className={combinedClassNames}>
      {children}
    </button>
  )
}
