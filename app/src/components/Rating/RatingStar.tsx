...
export const RatingStar: FC<RatingStarProps> = ({
  ...
  Icon =
    starType === 'half' ? (
      <RatingHalfStarType size={size} color={color} weight={filledType} />
    ) : (
      <RatingStarType size={size} color={color} weight={filledType} />
    )
...
const RatingStarType: FC<Rate> = ({ size, color, weight }) => {
  if (weight === 'regular') {
    return (
...
const RatingHalfStarType: FC<Rate> = ({ size, color, weight }) => {
  if (weight === 'regular') {
    return (
      <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} fill={color} viewBox="0 0 256 256">
