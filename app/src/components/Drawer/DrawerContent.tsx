...
const contentPosition: DrawerPositionProps = keepTw({
  ...
  position: {
    ...
    left: 'inset-y-0 left-0 h-full  border-r dark:border-r-metal-800 data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left sm:max-w-lg rounded-r-[10px]',
    right:
      'inset-y-0 right-0 h-full  border-l dark:border-l-metal-800 data-[state=closed]:slide-out-to-right data-[state=open]:slide-in-from-right sm:max-w-lg rounded-l-[10px]',
  },
})
...
const DrawerContent = forwardRef<ElementRef<typeof Content>, DrawerContentProps>(
  ({ className, children, position = 'bottom', ...props }, ref) => (
    <Portal>
      <DrawerOverlay />
      <Content ref={ref} className={cn(contentPosition.base, contentPosition.position[position], className)} {...props}>
        {children}
        <Close className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-white transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-metal-50 focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-white dark:fill-white dark:ring-offset-metal-900 dark:focus:ring-metal-800">
