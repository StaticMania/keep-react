...
Elevate the readability of your text with the Body Typography component in Keep React. Employ `<Typography>` with the `variant` prop, featuring six body text styles from `"body-1"` to `"body-5"`.
...
Make a visual impact with the Display Typography component in Keep React. Use `<Typography>` with the `variant` prop with `'display'`.
