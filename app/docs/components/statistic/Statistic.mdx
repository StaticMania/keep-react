import { DefaultStatistic, DefaultStatisticCode } from './variant/DefaultStatistic'
import { StatisticWithProgressRate, StatisticWithProgressRateCode } from './variant/StatisticWithProgressRate'
import { StatisticWithFilterButton, StatisticWithFilterButtonCode } from './variant/StatisticWithFilterButton'
import { StatisticWithAvatar, StatisticWithAvatarCode } from './variant/StatisticWithAvatar'
import { StatisticWithIcon, StatisticWithIconCode } from './variant/StatisticWithIcon'
import { StatisticWithDollarSign, StatisticWithDollarSignCode } from './variant/StatisticWithDollarSign'
import { statisticApiData } from './StatisticApi'

import CodePreview from '~/components/CodePreview'
import CssThemePreview from '~/components/CssThemePreview'
import ComponentApi from '~/components/ComponentApi'
import { useTheme } from '~/src/Keep/ThemeContex'

## Table of Contents

The Statistic component in the Keep React allows you to display statistical information in a visually appealing and informative manner. With customizable options for type, number format, and additional features, you can present data in a way that best suits your design requirements.

## Default Statistic

Default Statistic component displays a title with `<Statistic.Title>` component and a total number with `<Statistic.Amount>` component.

<CodePreview github="Statistic" code={DefaultStatisticCode}>
  <DefaultStatistic />
</CodePreview>

## Statistic With Progress Rate

The Statistic with Progress Rate component offers a concise representation of numerical data along with a corresponding rate. It displays a title (`<Statistic.Title>`) followed by the count (`<Statistic.Amount>`) and a rate with (`<Statistic.Rate>`).

<CodePreview github="Statistic" code={StatisticWithProgressRateCode}>
  <StatisticWithProgressRate />
</CodePreview>

## Statistic With Dollar Sign

The `showDollar` prop is set to `true`, which indicates that the dollar sign should be shown next to the amount.

<CodePreview github="Statistic" code={StatisticWithDollarSignCode}>
  <StatisticWithDollarSign />
</CodePreview>

## Statistic With Filter Button

The `showFilter` prop is set to `true`, indicating that a filter button should be displayed alongside the statistic.

<CodePreview github="Statistic" code={StatisticWithFilterButtonCode}>
  <StatisticWithFilterButton />
</CodePreview>

## Statistic With Avatar

The Statistic with Avatar component combines numerical data presentation with an accompanying avatar image.

<CodePreview github="Statistic" code={StatisticWithAvatarCode}>
  <StatisticWithAvatar />
</CodePreview>

## Statistic With Icon

The Statistic with Icon component seamlessly integrates a meaningful icon with essential numeric data.

<CodePreview github="Statistic" code={StatisticWithIconCode}>
  <StatisticWithIcon />
</CodePreview>

## API Reference

Explore the available props for the statistic component

<ComponentApi data={statisticApiData} />

## Theme

To learn more about how to customize the appearance of components, please see the Theme docs.

<CssThemePreview code={useTheme().theme.statistic} />
