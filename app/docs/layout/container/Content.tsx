      ...
      <table className="whitespace-no-wrap z-50 mt-10 w-full min-w-max table-auto overflow-hidden rounded-xl bg-white text-left lg:min-w-min">
        <thead>
          <tr>
            ...
            <th className="bg-metal-900 px-4 py-3 text-body-4 font-medium text-white">Breakpoint</th>
        ...
        <tbody className="align-baseline">
          {ContainerData.map((item) => (
            <tr key={item.id}>
              <td
                translate="no"
                className={item.name && `border-t border-metal-100 px-4 py-3 text-body-4 font-normal text-metal-500`}>
                {item.name}
              </td>
