  ...
  return (
    <div className="hero-area">
      <div className="relative z-10 mx-auto max-w-2xl text-center">
        <h1 className="mb-3 text-4xl font-semibold leading-8 tracking-[-1.75px] text-black/100 md:text-8xl lg:text-[56px] lg:leading-[66px]">
          Supercharge Your Web Development with
          <span className="hero-text block">Keep React</span>
        </h1>
        <p className="mx-auto max-w-xs text-sm text-slate-600 md:max-w-lg md:text-lg">
    ...
    <div className="relative">
      ...
      <div className="container">
        <div className="mx-auto mb-4 px-5 text-center md:max-w-[500px] md:px-0">
          <h2 className="mb-3 text-4xl font-semibold leading-8 tracking-[-1px] text-black lg:text-8xl lg:!leading-[44px]">
        ...
        <div className="my-14 grid grid-cols-1 gap-5 lg:grid-cols-2 xl:grid-cols-3">
          {ComponentList.map((item) => {
            return (
              <div
                key={item.id}
                className={`group relative flex h-[244px] w-full cursor-pointer items-center justify-center rounded-2xl border border-slate-200 bg-white md:border-slate-100`}>
                {item.component()}
                <Link
                  href={item.href}
        ...
        <div className="flex items-center justify-center">
          <Link
            href="/docs/getting-started/Introduction"
            className="flex items-center gap-2 rounded-md border border-slate-200 bg-slate-25 px-4 py-2.5 text-sm text-slate-900 transition-all duration-300 hover:bg-slate-900 hover:text-white md:px-6 md:py-3 md:text-base">
            View All Component
            <ArrowUpRight size={18} />
          </Link>
        </div>
      ...
      <div className="flex flex-col items-start justify-between gap-5 rounded-md bg-[url('https://staticmania.cdn.prismic.io/staticmania/59c3bc39-d8bc-4382-80e9-db3c7f10230d_community.svg')] bg-cover bg-center bg-no-repeat p-8 md:px-8 md:py-10 lg:flex-row lg:items-center lg:gap-0 lg:px-28 lg:py-12">
        ...
        <div>
          <a
            ...
            target="_blank"
            className="flex items-center gap-2 rounded-md  bg-slate-900 px-4 py-2.5 text-sm text-white hover:bg-slate-800 md:px-6 md:py-3 md:text-base">
