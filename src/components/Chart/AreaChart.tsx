  ...
  const RenderLineChart = (
    <ResponsiveContainer width="100%" height={350} className="mx-auto">
      <AreaChartCom
        ...
        <Area type={chartType} dataKey={dataKey} stroke="#1B4DFF" fillOpacity={1} fill="url(#colorUv)" />

        {showTooltip && <Tooltip cursor={false} content={<CustomTooltip />} />}
        {secondaryDataKey && (
          <Area type={chartType} dataKey={secondaryDataKey} stroke="#94ABFF" fillOpacity={1} fill="url(#colorPv)" />
        )}
      </AreaChartCom>
    </ResponsiveContainer>
  )
