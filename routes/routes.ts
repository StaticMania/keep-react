interface routerPath {
  id: number;
  name: string;
  href: string;
}

export const routes: routerPath[] = [
  { id: 1, name: "Alert", href: "/alert" },
  { id: 2, name: "Avatar", href: "/avatar" },
  { id: 3, name: "Avatar Group", href: "/avatarGroup" },
  { id: 4, name: "Accordion", href: "/accordion" },
  { id: 5, name: "Button", href: "/button" },
  { id: 6, name: "Button Group", href: "/buttonGroup" },
  { id: 7, name: "Badge", href: "/badge" },
  { id: 8, name: "BreadCrumb", href: "/breadcrumb" },
  { id: 9, name: "Card", href: "/card" },
  { id: 10, name: "Carousel", href: "/carousel" },
  { id: 11, name: "Play Button", href: "/playButton" },
  { id: 12, name: "Progress", href: "/progress" },
  { id: 13, name: "Notification", href: "/notification" },
  { id: 14, name: "Tooltip", href: "/tooltip" },
  { id: 15, name: "Modal", href: "/modal" },
  { id: 16, name: "Rating", href: "/rating" },
  { id: 17, name: "Spinner", href: "/spinner" },
  { id: 18, name: "Toast", href: "/toast" },
  { id: 19, name: "Slider", href: "/slider" },
  { id: 20, name: "Tabs", href: "/tabs" },
  { id: 21, name: "Sidebar", href: "/sidebar" },
  { id: 22, name: "Switch", href: "/switch" },
  { id: 23, name: "Skeleton", href: "/skeleton" },
  { id: 24, name: "Tag", href: "/tag" },
  { id: 25, name: "Table", href: "/table" },
  { id: 26, name: "Steps", href: "/step" },
  { id: 27, name: "Timeline", href: "/timeline" },
  { id: 28, name: "Empty", href: "/empty" },
  { id: 29, name: "Statistic", href: "/statistic" },
  { id: 30, name: "Upload", href: "/upload" },
  { id: 31, name: "Dropdown", href: "/dropdown" },
  { id: 32, name: "CheckBox", href: "/checkbox" },
  { id: 33, name: "Text Input", href: "/textInput" },
  { id: 34, name: "Radio", href: "/radio" },
  { id: 35, name: "Text Area", href: "/textArea" },
  { id: 36, name: "Number Input", href: "/numberInput" },
  { id: 37, name: "Search Bar", href: "/searchBar" },
  { id: 38, name: "Date Picker", href: "/datePicker" },
  { id: 39, name: "CheckBox Group", href: "/checkboxGroup" },
  { id: 40, name: "Tree", href: "/tree" },
  { id: 41, name: "Navbar", href: "/navbar" },
  { id: 42, name: "Area Chart", href: "/areaChart" },
];

export const gettingStartedRoutes: routerPath[] = [
  {
    id: 1,
    name: "Introduction",
    href: "/introduction",
  },
  {
    id: 2,
    name: "Installation",
    href: "/installation",
  },
];
